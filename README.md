# MPPT Solar Charge Controller Hardware

本仓库整理 PCB-02 光伏 MPPT 可调充电功率板的硬件发布资料，包含原理图 PDF、Gerber/钻孔生产文件和开源发布说明。

## 项目定位

该硬件用于光伏输入到电池/直流母线的 DC-DC 转换与 MPPT/CC-CV 充电控制验证。当前资料对应一版基于 STM32G474RET6、双 INA226 采样、CAN 收发器、同步 Buck/Boost 功率级和风扇/NTC/使能/故障接口的 MPPT 功率板。
...
- 复核功率路径、采样路径和控制接口。
- 交给 PCB 厂进行打样前检查。
- 作为 STM32G474 + RT-Thread MPPT 固件的硬件参考。

当前公开资料还不等同于完整 EDA 源工程，因为压缩包内主要是 Gerber/Drill 生产文件，不包含可编辑原理图/PCB 工程源文件。
